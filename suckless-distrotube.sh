...
Server = https://gitlab.com/dwt1/\$repo/-/raw/master/\$arch" | sudo tee -a /etc/pacman.conf
    ...
    if !(check_file_exist "dmenu"); then
        yes | sudo pacman -S dmenu-distrotube-git 
        cd /opt/dmenu-distrotube-git 
	sudo sed 's/pixelsize\=11/pixelsize\=16/' config.def.h | sudo tee -a temp && sudo mv temp config.def.h
	sudo sed 's/pixelsize\=8/pixelsize\=16/' config.def.h | sudo tee -a temp && sudo mv temp config.def.h
	sudo cp config.def.h config.h
        sudo make clean install
        echo "${GREEN}pacman install dmenu-distrotube${RESET}"
    fi
    if !(check_file_exist "dwm"); then
    ...
    if !(check_file_exist "dwmblocks"); then
